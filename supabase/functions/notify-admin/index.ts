...
<body style="margin:0;padding:0;background:#f3f4f6;font-family:Arial,sans-serif;">
  <div style="max-width:640px;margin:32px auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
    ...
    <div style="background:#1a4d2e;padding:28px 32px;text-align:center;">
      ...
      <p style="color:rgba(255,255,255,0.7);margin:8px 0 0;font-size:13px;">New Member Profile — Verification Required</p>
    </div>

    <!-- Alert Banner -->
    <div style="background:#fefce8;border-left:4px solid #f59e0b;padding:14px 24px;">
      <p style="margin:0;color:#92400e;font-size:13px;font-weight:600;">
        A new member account was created by an administrator on ${now}. Please review the full profile and passport data below for verification.
      </p>
    </div>
    ...
    <div style="background:#f9fafb;padding:20px 32px;border-top:1px solid #e5e7eb;text-align:center;">
      <p style="margin:0;color:#9ca3af;font-size:12px;">
        ...
        Login to the admin panel to activate this member's account or update their status.
      </p>
    </div>
  </div>
</body>
    ...
    const emailRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: [ADMIN_EMAIL],
        subject: `New Member — ${profile.first_name} ${profile.last_name} (Verification Required)`,
        html: emailHtml,
      }),
    });
